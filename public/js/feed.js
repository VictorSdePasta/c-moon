...
async function carregar() {
  await fetch(`/posts/fillSelect/pais`)
  ...
  await feed('0')
  await markLiked(idUser)
}

async function markLiked(idUser) {
  try {
    const r = await fetch(`/feedContent/markLiked/${idUser}`);
    if (!r.ok || r.status === 204) return;

    const dados = await r.json();

    dados.forEach(item => {
      const btn = document.getElementById(`btn${item.idPost}`);
      if (btn && item.liked == 1) {
        btn.classList.add("alrLiked", "marked");
      }
    });
  } catch (e) { }
}

async function feed(idPais) {
  let resposta = await fetch(`/feedContent/fill/${idPais}`)
  if (!resposta.ok) throw 'Houve um erro na API!';

  if (resposta.status == 204) {
    document.getElementById("posts").innerHTML = "Nenhum resultado encontrado.";
    return;
  }
  ...
  for (let i = 0; i < publicacoes.length; i++) {
    ...
    try {
      const respSubs = await fetch(`/feedContent/selPostSubgenres/${publicacao.idPost}`);
      if (respSubs.ok) {
        const subs = await respSubs.json();
        for (let j = 0; j < subs.length; j++) {
          pubSubgenres += `#${subs[j].postSubg} `;
        }
      }
    } catch (e) { }

    const pubAge = publicacao.postAge ? `#${publicacao.postAge} ` : "";
    const pubCountry = publicacao.postCountry ? `#${publicacao.postCountry} ` : "";
    const pubLocation = publicacao.postLocation ? `#${publicacao.postLocation} ` : "";
...
async function selectFeedCountry() {
  let selCou = selFeedCountry.value
  await feed(selCou)
  await markLiked(idUser)
}
...
function like(idPost) {
  let button = document.getElementById(`btn${idPost}`)
  let divPost = document.getElementById(`divLikes${idPost}`)
  let liked

  if (button.classList.contains("alrLiked") && button.classList.contains("marked")) {
    button.classList.remove(`marked`)
    divPost.innerHTML = Number(divPost.innerHTML) - 1
    liked = 0

    fetch(`/feedContent/updateLike/${idPost}/${idUser}`, {
      method: "PUT",
      headers: {
      ...
      if (resposta.ok) {
        ...
        throw ("Houve um erro ao tentar curtir! Código da resposta: " + resposta.status);
      }
    }).catch(function (resposta) {
    })
    ...
    fetch(`/feedContent/like/${idPost}/${idUser}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
