...
function feed(idPais) {
  fetch(`/feedContent/fill/${idPais}`).then(function (resposta) {
    ...
    resposta.json().then(function (resposta) {
      ...
      resposta.forEach(async function (publicacao) {
        await fetch(`/feedContent/selPostSubgenres/${publicacao.idPost}`)
          .then(resp => resp.json())
          .then(subs => {
            console.log('Select recebido', JSON.stringify(subs));
            ...
            let pubAge = publicacao.postAge ? `#${publicacao.postAge} ` : '';
            let pubCountry = publicacao.postCountry ? `#${publicacao.postCountry} ` : '';
            let pubLocation = publicacao.postLocation ? `#${publicacao.postLocation} ` : '';
      ...
      fetch(`feedContent/markLiked/${idUser}`).then(function (resposta) {
        if (!resposta.ok) throw 'Houve um erro na API!';

        resposta.json().then((resp) => {
          resp.forEach(publicacao => {
            let likeButton = document.getElementById(`btn${publicacao.idPost}`)
            if (publicacao.liked == 1) { likeButton.classList.add(`alrLiked marked`) }
          });
        })
      })
    });
...
function like(idPost) {
  ...
  if (button.classList.contains(`alrLiked marked`)) {
